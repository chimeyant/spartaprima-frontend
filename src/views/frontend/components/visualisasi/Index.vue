<template>
  <div class="mt-10">
    <v-container>
      <v-row>
        <h3 class="display-1 font-weight-bold">Visualisasi</h3>
      </v-row>
            ...
            <v-row>
              ...
              <v-col cols="8">
                <span class="mt-5 text-h6 animate__animated animate__fadeInUp animate__delay-1s">
                  Tampilkan hasil karya Visualisasi Data Anda di Open Data Banten!
                  Visualisasi Data terpilih akan dipublikasikan untuk dapat diakses oleh pengguna Open Data Banten
                </span>
          ...
          <v-col cols="12">
            <v-row class="pl-3 pr-3 pt-5">
              <span class="text-body-2 font-weight-medium">
                {{ datas.meta.total }} Data Visualisasi
              </span>
              <v-spacer>
              </v-spacer>
              <v-select
...
export default {
  ...
  methods: {
    ...
    fetchVisualisasi: async function () {
      try {
        let { data } = await this.http.get("katalog/visualisasi");
        this.datas = data;
        this.perPage = "10";
      } catch (error) {}
    },

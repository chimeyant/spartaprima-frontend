<template>
  <div class="mt-10">
    <v-container>
      <v-row>
        <h3 class="display-1 font-weight-bold">Videografis</h3>
      </v-row>
      <v-row class="mt-10 text-body-2">
        Lihat informasi data tertentu dalam bentuk video.
      </v-row>
      <v-row class="mt-16">
        <v-card
          outlined
            ...
            <v-row>
              ...
              <v-col cols="8">
                <span class="mt-5 text-h6 animate__animated animate__fadeInUp animate__delay-1s">
                  Tampilkan hasil karya Video Anda di Open Data Banten!
                  Video terpilih akan dipublikasikan untuk dapat diakses oleh pengguna Open Data Banten
                </span>
            ...
            <div>
              ...
              <v-row class="mt-5 justify-around">
                <v-col
                  ...
                  :key="index"
                >
                  <v-card @click="openVisualisasiDetail(item.id)">
                    <youtube
                      :video-id="item.video"
                      width=100%
                    />
...
export default {
  ...
  methods: {
    ...
    fetchVisualisasi: async function () {
      try {
        let { data } = await this.http.get("katalog/videografis");
        this.datas = data;
        this.perPage = "10";
      } catch (error) {}
    },
    openVisualisasiDetail: async function (val) {
      this.$router.push({
        name: "visualisasi-detail",
        params: {
          visualization_uuid: val,
        },
      });
    },
  },
